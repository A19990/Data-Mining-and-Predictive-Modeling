...
def add_kurt_skew(df: pd.DataFrame):
    df = df.select_dtypes(include = np.number)
    skewness = pd.Series(map(lambda x: df[x].skew(), df.columns), index = df.columns)
    kurtosis = pd.Series(map(lambda x: df[x].kurtosis(), df.columns), index = df.columns)
    statistics = df.describe().T
    statistics['skewness'] = skewness
    statistics['kurtosis'] = kurtosis
    return statistics[['mean','std','50%','skewness','kurtosis']].T
